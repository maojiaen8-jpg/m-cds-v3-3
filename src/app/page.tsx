...
import { Printer, Save, Waves, Share2, Eye, X, Calculator, Lock, Trash2, Calendar, Bell, Clock } from "lucide-react";
...
const calculateMCDS = (a: any) => {
  if (!a) return [];
  const DIST = [25, 50, 100, 200, 400], SF: any = { Free:1, Back:1.06, Fly:1.12, Breast:1.18 };
  const DF: any = { Free:{100:1.01,200:1.02,400:1.03}, Back:{100:1.01,200:1.02,400:1.03}, Fly:{100:1.03,200:1.07,400:1.15}, Breast:{100:1.025,200:1.06,400:1.12} };
  // 锁死 RI 逻辑：根据模块强度定义
  const Z: any = { 
  ...
  return Object.keys(Z).map(z => {
    ...
    return { zone: z, paces: DIST.map(d => {
      ...
      return { v: formatPace(s), r: `${formatPace(s*0.98)}~${formatPace(s*1.02)}`, ri: Z[z].ri(d) };
    }), hr: Math.round(((220-a.age)*Z[z].h)/6) };
  });
};
...
export default function Page() {
  ...
  const loadS = async (cid: string) => { const { data } = await supabase.from("schedules").select("*").eq("coach_id", cid); if(data) setDbSched(data); };
  ...
  return (
    <div className="mcds-root">
      ...
      {role === "guest" && <div className="glass" style={{marginTop:'20vh', textAlign:'center'}}><Waves size={50} color="#facc15" style={{margin:'0 auto 20px'}}/><h2>M-CDS ELITE</h2><button className="btn-gold" style={{width:'100%'}} onClick={()=>setRole("coach_login")}>进入系统</button></div>}
      ...
      {role === "coach" && (
        <div className="glass no-print">
          ...
          {showCal && <div style={{padding:15, background:'#000', borderRadius:15, border:'1px solid #facc15', marginBottom:20}}>
            ...
            <div className="cal-grid">{Array.from({length:28}).map((_, i) => { const s=dbSched.find(d=>d.day_index===i+1); return (<div key={i} className={`cal-day ${selDay===i+1?'active':''} ${s?.athlete_ids?.length>0?'scheduled':''}`} onClick={()=>setSelDay(i+1)}>D{i+1}{s?.athlete_ids?.length>0 && <div className="indicator"></div>}</div>)})}</div>
            {selDay && (
              <div style={{marginTop:10, borderTop:'1px solid #222', paddingTop:10}}>
                <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(90px, 1fr))', gap:5}}>
                  {athletes.map(a => {
                    ...
                    return <button key={a.id} onClick={async()=>{
                      ...
                      const newIds = isSet ? d.athlete_ids.filter((id:any)=>id!==a.id) : [...d.athlete_ids, a.id];
                      await supabase.from("schedules").upsert({...d, athlete_ids:newIds, coach_id:user.id}); loadS(user.id);
                ...
                <textarea style={{width:'100%', marginTop:10, height:40, background:'#000', border:'1px solid #333', color:'#fff', borderRadius:8, padding:8}} placeholder="今日训练内容..." value={dbSched.find(s=>s.day_index===selDay)?.content||''} onChange={async(e)=>{ const d=dbSched.find(s=>s.day_index===selDay)||{day_index:selDay,athlete_ids:[],content:''}; await supabase.from("schedules").upsert({...d, content:e.target.value, coach_id:user.id}); loadS(user.id); }} />
              </div>
            )}
          </div>}
          ...
          <div style={{width:'100%', maxWidth:850, background:'#0a0c10', padding:20, borderRadius:24, position:'relative', height:'fit-content'}}>
            ...
            <div style={{marginBottom:15}}>
              ...
              <div className="cal-grid">{Array.from({length:28}).map((_, i) => {
                ...
                return <div key={i} className={`cal-day ${isSet?'scheduled':''}`} onClick={()=>isSet && alert(`今日计划：\n${s.content || '正常训练'}`)}>D{i+1}{isSet && <div className="indicator"></div>}</div>
              })}</div>
            </div>
